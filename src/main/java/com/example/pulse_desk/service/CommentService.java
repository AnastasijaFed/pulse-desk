...
public class CommentService {
    ...
    public CommentResponse submitComment(String content){
        ...
        analyzeCommentAsync(comment.getId());
    ...
    public void analyzeCommentAsync(Long commentId){
        log.info("Starting async analysis for comment {}", commentId);
        ...
        try {
            ...
            if (decision.shouldCreateTicket()) {
                ...
                ticketService.createNewTicket(
                    decision.title(), 
                    decision.summary(), 
                    TicketCategory.valueOf(decision.category().toUpperCase()), 
                    TicketPriority.valueOf(decision.priority().toUpperCase()), 
                    comment.getId()
                );
        ...
        commentRepository.save(comment);
        log.info("Async analysis completed for comment {}", commentId);
    }
